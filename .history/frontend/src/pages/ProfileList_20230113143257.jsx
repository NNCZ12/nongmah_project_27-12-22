  ...
  return (
    <>
      ...
      {/* Dog List */}{" "}
        ...
        <Col>{profiles.map(profiles=>(
          <Row key={profiles.id}>
            <Container>
            <img src={`http://localhost:8000/storage/dog_profiles/image/${profiles.image}`} alt="" />
            </Container>
          </Row>
        );)}
        </Col>
      {/* <Container>{profiles.length > 0?(profiles.map((row, key)=> (
        <Row  key={key}>
        <Col>{row.name}</Col>
        <Col><img src={`http://localhost:8000/storage/dog_profiles/image/${row.image}`} alt="" /></Col>
      </Row>
      ))):(<p>No Dogs Found</p>)}
