...
axios.post('http://localhost:5000/image', formData)
  ...
  .catch(err => {
   ...
   axios.get('http://localhost:5000/image')  // make a GET request to the API endpoint for image retrieval 
     .then(res => { // handle success response from the API call and set the image URL in state using setImageURL() method 

       setImageURL(res.data); // res.data will contain the image URL returned by the API call

     }) 

     .catch(err => { // handle error response from the API call and show an error message using SweetAlert2 library

       Swal.fire({  // show an error message using SweetAlert2 library if there is an error while retrieving image from the API
