...
function ProfileForm(props) {
  ...
  const profileData = [latitude,longitude,image,vaccine,spotOn,neuter,gender,dog_character,appearance,location,color];
  const allowSubmit = () => {profileData.map((profileData)=>{profileData ?})}
  ...
  const getButtonText = () => {
    return image ? 'อัปโหลดรูปภาพแล้ว' : 'อัปโหลดรูปภาพน้องหมา'
  }
      ...
      <Form onSubmit={createProfile}>
        ...
        <Form.Group controlId="Image" className="mb-3">
          ...
          >
            ...
            <Modal.Body>
              <ChooseImage handleGetImage={handleGetImage} handleCloseImage={handleCloseImage}/>
            </Modal.Body>
              ...
              onChange={(date) => {
                if (date === null) {
                  ...
                  const vaccineDate = new Date(
                    ...
                    .replace('T', ' ')
                    .replace('Z', ' ')
                  setSelectVaccineDate(vaccineDate)
                }
              }}
        ...
        {spotOn === 'received' && (
          <Form.Group>
            <DatePicker
              ...
              onChange={(date) => {
                if (date === null) {
                  setSelectSpotOnDate('0000-00-00 00:00:00.000')
                } else {
                  const spotOnDate = new Date(
                    date - new Date().getTimezoneOffset() * 60000,
                  )
                    .toISOString()
                    .replace('T', ' ')
                    .replace('Z', ' ')
                  setSelectVaccineDate(spotOnDate)
                }
              }}
